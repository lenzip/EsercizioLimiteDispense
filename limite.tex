...
included in the fitting procedure in the of parameters for which an external
...
for $\mu$, $\hat{\mu}$. The result id the already known one of Eq~\ref{eq:nuhat}, 
...
We can the add to our likelihood a nuisance parameter $\mu_b$ which acts as a
multiplier of $\nu_b$ in much the same way as $\mu$ is multiplier for the
number of expected signal events. However we impose a (for example) Gaussian
constraint on $\nu_b$ with standard deviation $\delta_{\nu_b}/\nu_b$. The new
...
extended to the case in which we our measurement consists of a vector of
...
background process), each with its own normalization uncertainty. In this case one simply add each nuisance as a
