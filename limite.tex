...
\begin{equation}
\nu_{up} = \hat{\nu}_s(M) + \delta_{\hat{\nu}_s(M)}\cdot\Phi^{-1}(0.95)
\label{eq:limitcowan}
\end{equation}
where $\hat{\nu}_s(M)$ is given by Eq.~\ref{eq:nuhat}, $\delta_{\hat{\nu}_s(M)}$ 
is the error on $\hat{\nu}_s(M)$, $\Phi^{-1}$ is the inverse cumulative distribution 
of the Gaussian.

Eq.~\ref{eq:limitcowan} {\bf holds under the assumption that the distribution of
$\hat{\nu}_s(M)$ is Gaussian}. However $\hat{\nu}_s(M)$, as defined in
...
assumption that $\hat{\nu}_s(M)$ is Gaussian is only valid in case $N_{obs}$
...
How do we estimate $\delta_{\hat{\nu}_s(M)}$? If we know the number of
expected background events with no error, then
$\delta_{\hat{\nu}_s(M)}=\sqrt{N_{obs}}$, following Poisson statistics. 

However we usually have an uncertainty on the estimation of the number of
background events. This kind of uncertainty is systematic, contrary to the
